...
- Para la coordinacion entre sum workers se tuvo que reducir el pre-fetch de pika a 1 mensaje, el pre-fetch en este caso es un sistema que implementa la libreria de pika en python para hacer un buffer de una cantidad especifica de mensajes para liberar la queue de rabbit y mejorar el throughput del sistema, esto se hizo principalmente porque por como idee la arquitectura era supuseto que si algun sum worker recibia el eof entonces los otros worker ya habian terminado de procesar toda la info recibida de ese cliente. La desventaja de hacer esto es que empora el throughput y aumnenta el tiempo de espera entre mensajes de cada sum worker aciendo que el sistema sea mas lento.