...
- Para la coordinación entre sus workers se tuvo que reducir el prefetch de pika a 1 mensaje, el prefetch en este caso es un sistema que implementa la librería de pika en python para hacer un buffer de una cantidad específica de mensajes para liberar la queue de rabbit y mejorar el throughput del sistema, esto se hizo principalmente porque por como idee la arquitectura era supuesto que si algun sum worker recibía el eof entonces los otros worker ya habían terminado de procesar toda la info recibida de ese cliente. La desventaja de hacer esto es que empeora el throughput y aumenta el tiempo de espera entre mensajes de cada sum worker haciendo que el sistema sea más lento.
